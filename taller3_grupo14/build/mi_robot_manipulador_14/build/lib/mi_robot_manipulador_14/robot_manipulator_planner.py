...
class nodoQueSeSuscribe(Node):
    def __init__(self):
        ...
        self.nodito = self.create_subscription(Vector3, '/robot_manipulator_goal',self.listener_callback,10)
    ##### SUSCRIBER CALLBACK #####
    def listener_callback(self, msg):
        global equis, ye, zeta
        print('La posición es =  x: '+str(msg.x)+'; y: ' + str(msg.y)+'; z: ' + str(msg.z))
        equis = msg.x
        ye = msg.y
        zeta = msg.z
        update_graph(msg.x,msg.y, msg.z)
        ...
        theta0, theta1, theta2 = inverse_kinematics(equis, ye, zeta, L1, L2)
        sendSize = 0
...
def inverse_kinematics(x, y, z, L1, L2):
    theta0 = round(math.atan2(z, x),2)
    L = math.sqrt(x ** 2 + y ** 2)
    alpha = math.acos((L1 ** 2 + L ** 2 - L2 ** 2) / (2 * L1 * L))
    theta1 = round(math.atan2(y, x) + alpha, 2)
    theta2 = round(math.acos((L1 ** 2 + L2 ** 2 - L ** 2) / (2 * L1 * L2)), 2)
    return theta0, theta1, theta2

def update_graph(x_input, y_input, z_input):
    ...
    z_target = float(z_input)
    ######################################################################################################################################
    ##Estos son los angulos que hay que enviar al arduino como un mensaje: "punto3, theta0, theta1, theta2"
    #######################################################################################################################################
    estructuraTransmision = structForTransmition
    theta0, theta1, theta2 = inverse_kinematics(x_target, y_target, z_target, L1, L2)
    sendSize = 0
    estructuraTransmision.th0_vel = theta0 * (180/math.pi)
    estructuraTransmision.th1 = theta1 * (180/math.pi)
    estructuraTransmision.th2 = theta2 * (180/math.pi)
...
button_update = tk.Button(window, text="Actualizar", command=lambda: update_graph(equis, ye, zeta))
