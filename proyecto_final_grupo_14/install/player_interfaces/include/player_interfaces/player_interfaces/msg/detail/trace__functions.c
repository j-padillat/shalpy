// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from player_interfaces:msg/Trace.idl
// generated code does not contain a copyright notice
#include "player_interfaces/msg/detail/trace__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `key`
#include "rosidl_runtime_c/string_functions.h"

bool
player_interfaces__msg__Trace__init(player_interfaces__msg__Trace * msg)
{
  if (!msg) {
    return false;
  }
  // key
  if (!rosidl_runtime_c__String__init(&msg->key)) {
    player_interfaces__msg__Trace__fini(msg);
    return false;
  }
  // x
  // y
  // th
  // time
  return true;
}

void
player_interfaces__msg__Trace__fini(player_interfaces__msg__Trace * msg)
{
  if (!msg) {
    return;
  }
  // key
  rosidl_runtime_c__String__fini(&msg->key);
  // x
  // y
  // th
  // time
}

bool
player_interfaces__msg__Trace__are_equal(const player_interfaces__msg__Trace * lhs, const player_interfaces__msg__Trace * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // key
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->key), &(rhs->key)))
  {
    return false;
  }
  // x
  if (lhs->x != rhs->x) {
    return false;
  }
  // y
  if (lhs->y != rhs->y) {
    return false;
  }
  // th
  if (lhs->th != rhs->th) {
    return false;
  }
  // time
  if (lhs->time != rhs->time) {
    return false;
  }
  return true;
}

bool
player_interfaces__msg__Trace__copy(
  const player_interfaces__msg__Trace * input,
  player_interfaces__msg__Trace * output)
{
  if (!input || !output) {
    return false;
  }
  // key
  if (!rosidl_runtime_c__String__copy(
      &(input->key), &(output->key)))
  {
    return false;
  }
  // x
  output->x = input->x;
  // y
  output->y = input->y;
  // th
  output->th = input->th;
  // time
  output->time = input->time;
  return true;
}

player_interfaces__msg__Trace *
player_interfaces__msg__Trace__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  player_interfaces__msg__Trace * msg = (player_interfaces__msg__Trace *)allocator.allocate(sizeof(player_interfaces__msg__Trace), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(player_interfaces__msg__Trace));
  bool success = player_interfaces__msg__Trace__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
player_interfaces__msg__Trace__destroy(player_interfaces__msg__Trace * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    player_interfaces__msg__Trace__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
player_interfaces__msg__Trace__Sequence__init(player_interfaces__msg__Trace__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  player_interfaces__msg__Trace * data = NULL;

  if (size) {
    data = (player_interfaces__msg__Trace *)allocator.zero_allocate(size, sizeof(player_interfaces__msg__Trace), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = player_interfaces__msg__Trace__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        player_interfaces__msg__Trace__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
player_interfaces__msg__Trace__Sequence__fini(player_interfaces__msg__Trace__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      player_interfaces__msg__Trace__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

player_interfaces__msg__Trace__Sequence *
player_interfaces__msg__Trace__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  player_interfaces__msg__Trace__Sequence * array = (player_interfaces__msg__Trace__Sequence *)allocator.allocate(sizeof(player_interfaces__msg__Trace__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = player_interfaces__msg__Trace__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
player_interfaces__msg__Trace__Sequence__destroy(player_interfaces__msg__Trace__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    player_interfaces__msg__Trace__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
player_interfaces__msg__Trace__Sequence__are_equal(const player_interfaces__msg__Trace__Sequence * lhs, const player_interfaces__msg__Trace__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!player_interfaces__msg__Trace__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
player_interfaces__msg__Trace__Sequence__copy(
  const player_interfaces__msg__Trace__Sequence * input,
  player_interfaces__msg__Trace__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(player_interfaces__msg__Trace);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    player_interfaces__msg__Trace * data =
      (player_interfaces__msg__Trace *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!player_interfaces__msg__Trace__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          player_interfaces__msg__Trace__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!player_interfaces__msg__Trace__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
